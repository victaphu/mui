...
export default function SellBuyNowPopup({
  ...
  const listItemAsFixedPrice = async () => {
    ...
    if (!approved.error) {
      ...
      const result = await marketplace.fixedPrice(
        account,
        tradeAsset.token_id,
        inputPrice,
        endTime,
  ...
  return (
    <Popup closePopup={closePopupAndClear} backClick={backClick} title="Listing Buy now">
      ...
      {tradeAssetStatus != 'complete' && (
        <>
          <p className="font-normal text-center mb-2 px-2">
            Set the sale price you want to list your NFT for.
          </p>
